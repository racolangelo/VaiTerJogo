 ...
 *
 *
 * (Mod) -> Modificações por Gabriel Flaeck Neves (gabriel@brivia.com.br)
 */

jQuery.fn.ingrid = function(o){
	
	var regexpurl = /^[\.\.\/]+/; // (Mod) - Expressão Regular para substituir '../' por '' //
	var restbl = ''; // (Mod) - Variável para controle de exibição da paginação //
	...
	var cfg = {
		...
		// (Mod) - Carrega funções específicas passadas por parâmetro //		
		loadComplete: '',
	...
	h.find('th').each(function(i){
		...
		if (cfg.sorting) {
			...
			col_label.addClass(cls).click(function(){
				...
				// (Mod) - Removendo '../' da string com o endereço a ser carregado por Ajax
				cfg.url = cfg.url.replace(regexpurl, '');
	...
	if (cfg.paging) {
			...
			pv = jQuery('<div />')
						...
						.extend({
							updateViewInfo : function(loaded_rows, page){
								...
								this.html('Visualizando linhas ' + _start + ' - ' + _end + ' de ' + cfg.totalRecords);
								return this;
							}
						});
		...
		// start page button
		// (Mod) - O link para o qual a função abaixo aponta foi alterado de '#' para 'javascript:void(0)'
		...
		var pform = jQuery('<form></form>').append(pinfo).submit(function(){
									...
									if (_p) {
										...
										alert('Informe um n&uacute;mero de p&aacute;gina v&aacute;lido.');
									}
									return false;
								});
		...
		if (cfg.totalRecords > 0) {
			...
			pinfo.html('P&aacute;gina ' + pinfo.html() + ' de ' + totp);
			
			// (Mod) - O link para o qual a função abaixo aponta foi alterado de '#' para 'javascript:void(0)'
			var pb4		= jQuery('<a href="javascript:void(0);">&raquo;</a>').addClass(cfg.pageEndClass).click(function(){
			...
			pinfo.html('P&aacute;gina ' + pinfo.html());
		}
	...
	g.extend({
		load : function(params, cb) {
			...
			// (Mod) - Inibe a barra de paginação durante o carregamento da tabela
			// $(".grid-page-toolbar").hide();
			...
			jQuery.ajax({
				...
				success: function(result){						
					...
					if (cfg.dataType == 'html') {
						...
						restbl = $tbl; // (Mod) - Seta a váriavel de controle de exibição da paginação
						...
						if ( jQuery(row).find('td').length == cfg.colWidths.length ) {
							...
							alert('Erro: Nenhuma linha retornada.');
						} else {
							...
							alert('Erro: Total de colunas retornado [' + $tbl.find('tbody tr:first td').length + '] diferente do passado ao Ingrid ['+ cfg.colWidths.length +'].');
						}
					}
					if (cb) cb();
				},
				error: function(){
					...
					alert('Erro: N&atilde;o foi poss&iacute;vel carregar "' + cfg.url + '". Cheque a URL e tente novamente. ');
				},
				complete: function(){
					modalmask.hide();
					
					// (Mod) - Inibe/exibe a barra de paginação de acordo com o número de páginas //
					if (cfg.dataType == 'html')
					{
						if(cfg.recordsPerPage != '' && cfg.recordsPerPage > 0)
						{
							var pag_atual = p.getPage();
							var total_linhas = restbl.find('tr').length;
							
							if (pag_atual == 1 && total_linhas < cfg.recordsPerPage)
							{
								$(".grid-page-toolbar").hide();
							}
							/*
