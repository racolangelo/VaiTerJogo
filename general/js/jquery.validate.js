...
// adiciona ao plugin validate do Jquery o m�todo que valida CPF
jQuery.validator.addMethod("cnpf", function( value, element )
{
	var expreg = /[ .\/\-]+/g;
	var valor = value;
	...
	return result;
}, "CNPF inv&aacute;lido");

// adiciona ao plugin validate do Jquery o m�todo que valida CNPJ
...
// adiciona ao plugin validate do Jquery o m�todo que valida CPF
jQuery.validator.addMethod("diferente", function( value, element, params )
{
	if (value == params)
	{
		return false;
...
// VALIDA��O REAL DE DATA
	
//Formato BR (dd/mm/aaaa)
//^(((0[1-9]|[1-2][0-9]|30)/(0[469]|11))|((0[1-9]|[1-2][0-9]|3[0-1])/(0[13578]|(10|12)))|((0[1-9]|[1-2][0-9])/02))/[0-9]{4}$

//Formato ISO (aaaa-mm-dd)
//^[0-9]{4}-(((0[13578]|(10|12))-(0[1-9]|[1-2][0-9]|3[0-1]))|(02-(0[1-9]|[1-2][0-9]))|((0[469]|11)-(0[1-9]|[1-2][0-9]|30)))$
jQuery.validator.addMethod("data", function( value, element, params )
{
	if(value.length > 0 && value != "__/__/____")
	{
		var expressao = "^(((0[1-9]|[1-2][0-9]|30)/(0[469]|11))|((0[1-9]|[1-2][0-9]|3[0-1])/(0[13578]|(10|12)))|((0[1-9]|[1-2][0-9])/02))/[0-9]{4}$";
		var re = new RegExp(expressao);
		
		var resultado = re.test(value);
...
jQuery.validator.addMethod("dataISO", function( value, element, params )
{
	var expressao = "^[0-9]{4}-(((0[13578]|(10|12))-(0[1-9]|[1-2][0-9]|3[0-1]))|(02-(0[1-9]|[1-2][0-9]))|((0[469]|11)-(0[1-9]|[1-2][0-9]|30)))$";
	var re = new RegExp(expressao);
...
/**
* Faz o date range entre 2 campos date.
**/
jQuery.validator.addMethod("dataComparada", function( value, element, params )
{
	var dataCampo = $.trim(value);
	var dataComparacao = params;
	var operacao;
	
	if (dataCampo == "" || dataCampo == "__/__/____")
	{
		return true;
	}
	
	dataComparacao = dataComparacao.split("|");
	operacao = dataComparacao[1];
	
	dataComparacao = $(dataComparacao[0]).val();
	
	dataCampo = dataCampo.split("/");
	dataCampoN = new Date();
	dataCampoN.setYear(dataCampo[2]);
	dataCampoN.setMonth(dataCampo[1]);
	dataCampoN.setDate(dataCampo[0]); // javascript: new Date(ano, mes, dia);
	
	//data1 = dataCampoN.getTime();
	data1 = new Date(dataCampo[2], dataCampo[1], dataCampo[0]);
	data1 = data1.getTime();
	
	dataComparacao = dataComparacao.split("/");
	
	if (dataComparacao.length > 0)
	{
		dataCompara = new Date();
		dataCompara.setYear(dataComparacao[2]);
		dataCompara.setMonth(dataComparacao[1]);
		dataCompara.setDate(dataComparacao[0]); // javascript: new Date(ano, mes, dia);
		data2 = dataCompara.getTime();

		strComparar  = "if ("+data1+" "+ operacao +" "+data2+")";
		strComparar += "{";
		strComparar += "	passou = 'sim';";
		strComparar += "}";
		strComparar += "else";
		strComparar += "{";
		strComparar += "	passou = 'nao';";
		strComparar += "}";
